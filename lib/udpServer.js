...
UdpServer.prototype.start = function () {
  // watch system memory
  this.memcheckTimeout = setTimeout(memCheck.bind(this), 30000).unref();
  ...
  this.server.on('close', function () {
    if (this.address) {
      ...
      log.log('close udpServer (listen address unknown)');
    }
  }.bind(this));
...
function messageHandler(msg, rinfo) {
  ...
  // remove illegal characters from the routing key
  msg.routingKey = msg.routingKey.replace(/[^-\.\w]/g, '');
