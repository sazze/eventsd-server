...
Consumer.prototype.stopConsume = function (socket, data) {
  ...
  _.forEach(this.consumers[socket.id].routingKeys[data.routingKey].subscriptions, function (subscription, topicName) {
    ...
    subscription.exists(function (err, exists) {
      ...
      if (!exists) {
        // no need to delete a subscription that does not exist
        log.debug('subscription for topic ' + topicName + ' already deleted');
        return;
      }
      ...
      subscription.delete(function (err, apiResponse) {
        ...
        log.debug('subscription for topic ' + topicName + ' deleted');
      });
    });
  });
